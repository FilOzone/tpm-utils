...
After this feature:
1. A new shared package `ghprojects-client` contains all project board client logic
2. Both `filozzy-mcp` and `foc-pr-report` import from `ghprojects-client`
...
│   └── tests/
│       └── test_integration.py
├── filozzy-mcp/                 # MODIFIED: thinner, imports from ghprojects_client
│   ├── pyproject.toml           # depends on ghprojects-client instead of foc-pr-report
│   ├── filozzy_mcp/
│   │   ├── server.py            # MCP tools + config + formatting
│   │   └── action_log.py        # Audit logging (stays here)
│   └── tests/
│       └── test_integration.py
├── foc-pr-report/               # MODIFIED: imports from ghprojects_client
...
# Or individually
cd ghprojects-client && GITHUB_TOKEN=$(gh auth token) uv run pytest -v
...
1. **Create `ghprojects-client`** — extract shared code, keep old `foc_project14_client.py` as re-export shim
2. **Update `filozzy-mcp`** — import from `ghprojects_client`, add env-based board config
3. **Update `foc-pr-report`** — import from `ghprojects_client`, move PR-specific code
