...
class GitHubPRReporter:
    ...
    def get_repo_prs(
        self, repo: str, state: str = "open", cutoff_date: datetime = None
    ) -> List[Dict[str, Any]]:
        ...
        while page <= max_pages:
            ...
            try:
                ...
                if cutoff_date and state == "closed":
                    oldest_pr_date = datetime.strptime(
                        prs[-1]["updated_at"], "%Y-%m-%dT%H:%M:%SZ"
                    )
                    if oldest_pr_date < cutoff_date:
                        ...
                        filtered_prs = [
                            pr
                            for pr in prs
                            if datetime.strptime(pr["updated_at"], "%Y-%m-%dT%H:%M:%SZ")
                            >= cutoff_date
                        ]
...
def main():
    ...
    parser.add_argument(
        "repos", nargs="+", help="GitHub repositories in format owner/repo"
    )
    parser.add_argument(
