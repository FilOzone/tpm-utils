    ...
    def generate_report(self, usernames: List[str], months: int = None) -> str:
        ...
        for username in usernames:
            ...
            for pr in user_prs:
                ...
                if (
                    cutoff_date is None
                    or formatted_pr["updated_datetime"] >= cutoff_date
                ):
        ...
        report.append(
            f"Total: {len(all_prs)} open PRs across {len(user_counts)} team members"
        )

        return "\n".join(report)

    def _add_user_section(
        self, report: List[str], username: str, user_prs: List[Dict[str, Any]]
    ):
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Only show PRs modified in the last N months (default: all PRs)",
    )
