    ...
    def search_and_collect(self, query: str, count: int = 10) -> QueryResults:
        ...
        if self.excluded_urls:
            ...
            messages = [
                msg
                for msg in messages
                if msg.get("permalink") not in self.excluded_urls
            ]
            filtered_count = original_count - len(messages)
            if filtered_count > 0:
                print(
                    f"Filtered out {filtered_count} excluded messages", file=sys.stderr
                )
        ...
        for i, message in enumerate(messages, 1):
            ...
            # Get channel and user names
            channel_name = (
                self.get_channel_info(channel_id) if channel_id else "Unknown"
            )
            user_name = self.get_user_info(user_id) if user_id else "Unknown"
        ...
        print(
            f"Cache stats - Channels: {len(self.channel_cache)}, Users: {len(self.user_cache)}",
            file=sys.stderr,
        )
    ...
    def to_markdown(session: SearchSession) -> str:
        ...
        for i, query_results in enumerate(session.queries, 1):
            ...
            for j, result in enumerate(query_results.results, 1):
                ...
                toc.append(
                    f"   {i}.{j} [{date} - {result.channel_name} - {result.user_name}](#{result_anchor})"
                )
        ...
        for query_results in session.queries:
            ...
            output.append(
                f"**Showing:** {len(query_results.results)} most recent results"
            )
            output.append("")

            for result in query_results.results:
                output.append(
                    f"### {query_results.query} Result {result.result_number}"
                )
                output.append("")
...
def main():
    ...
    parser.add_argument(
        "--token", help="Slack User OAuth Token (or set SLACK_USER_TOKEN env var)"
    )
    parser.add_argument(
        "--workspace",
        default="filecoinproject",
        help="Slack workspace name (default: filecoinproject)",
    )
    ...
    parser.add_argument(
        "--exclude-url",
        action="append",
        help="Exclude messages with these URLs from results (can be used multiple times)",
    )
    parser.add_argument(
        "--exclude-urls-file", help="File containing URLs to exclude (one per line)"
    )
    parser.add_argument(
        "--delay",
        type=float,
        default=0.5,
        help="Delay between search queries in seconds (default: 0.5)",
    )
    ...
    parser.add_argument(
        "--import-json",
        help="Import search results from JSON file and convert to markdown",
    )
    parser.add_argument(
        "--format",
    ...
    parser.add_argument(
        "queries", nargs="*", help="Search queries (or read from stdin)"
    )
    ...
    if not token:
        print(
            "Error: Slack user token required. Set SLACK_USER_TOKEN environment variable or use --token flag."
        )
        ...
        print(
            "Note: User tokens can search all channels you have access to (including private channels)"
        )
        sys.exit(1)
    ...
    if args.exclude_urls_file:
        try:
            ...
            print(
                f"Error: Exclude URLs file not found: {args.exclude_urls_file}",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    for i, query in enumerate(queries):
        try:
            ...
            if i > 0:
                print(
                    f"Waiting {args.delay} seconds before next query...",
                    file=sys.stderr,
                )
