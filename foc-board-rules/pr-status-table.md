...
| `last_feedback` | Timestamp of the most recent *actionable* human feedback: CHANGES_REQUESTED or COMMENTED reviews, plus substantive PR-level comments from write/maintain/admin humans (`authorAssociation` of OWNER/MEMBER/COLLABORATOR). Excludes: APPROVED reviews (an approval is not feedback the author must act on), the PR author's own reviews and comments (self-reviews are not reviewer engagement), bot and `app/*` authors, and pure coordination chatter (merge timing, status updates). `null` if no such feedback exists. |
