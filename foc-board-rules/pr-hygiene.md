...
**How to check:** Use `gh pr view -R <repo> <number> --json reviews,commits,comments --jq '{reviews: [.reviews[] | {author: .author.login, state: .state, submittedAt: .submittedAt}], lastCommit: .commits[-1].committedDate}'` to get the timestamps the table's inputs need. Only needed for PRs where the per-repo `gh pr list` data shows human review engagement.
