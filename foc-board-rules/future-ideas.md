...
**New sub-problem this surfaced:** rules that need to know "did I already do this, and did a human undo it" (R-FC-012's Cycle removal-guard) can't ask GitHub — there's no change-history API for Projects v2 custom fields other than Status. `foc-mechanical-rules` now keeps its own mutation history (a TSV file, `state.py`) for this, persisted across hourly runs via GitHub Actions cache. That's a reasonable v1, but a GitHub Actions cache isn't a truly durable store (eviction after ~7 days unused, no cross-repo/cross-workflow access). If more rules end up depending on this history, or if cache eviction ever causes a visible miss, worth moving it somewhere durable — a small persisted DB/file the REST API server owns, or simply committing the state file to the repo (rejected for v1 to avoid hourly bot commits, but revisit if the cache proves unreliable).
