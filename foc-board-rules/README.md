...
Rules for keeping the [FilOzone FOC project board](https://github.com/orgs/FilOzone/projects/14) consistent and up-to-date. These rules are intended to be applied by an LLM (via the board REST API, discovered through the FilOzzy MCP coordinator) or by a human during board triage.
...
6. **Supplement board data with GitHub PR metadata in two phases.** The project board provides field values (Status, Cycle Theme, Milestone, etc.) but not PR-specific metadata like author, draft status, reviewer assignments, or review decisions. Many rules (R-PR-001, R-PR-002, R-PR-005, R-PR-006, R-PR-007, R-SL-001) need this metadata. Use a two-phase approach to keep context lean.
   ...
   **Phase 2 — targeted per-PR deep dive.** For specific PRs identified by cross-referencing Phase 1 data with board status, run `gh pr view -R <repo> <number> --json reviews,commits,reviewRequests,comments` (include `comments` for R-SL-010). Phase 2 candidates:
   ...
   - **R-SL-010 (unaddressed reviewer comments):** All Awaiting Review PRs need a comment check. PRs already in Phase 2 above get `comments` for free (included in the `--json` field list). Remaining Awaiting Review PRs get a lightweight **Phase 2b** call: `gh pr view -R <repo> <number> --json comments,commits` (no `reviews` — only serves R-SL-010). Filter comments to those with `authorAssociation` of `OWNER`, `MEMBER`, or `COLLABORATOR` — skip community/external comments per R-SL-010's skip condition.

   **Batch all Phase 2 candidates.** Collect the union of candidates from all rules above into a single list before fetching. Run all `gh pr view` calls in parallel (both Phase 2 and Phase 2b) — don't fetch per-rule sequentially. This avoids duplicate lookups when the same PR is a candidate for multiple rules (e.g., a PR in Awaiting Review with empty `reviewDecision` and empty `reviewRequests` is a candidate for both R-PR-006 and R-PR-007).
