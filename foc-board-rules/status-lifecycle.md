...
**Canonical routing:** This rule is row 3 of the [PR status determination table](pr-status-table.md); the verification, superseding, and re-request logic below defines that table's `authoritative_approval` and `blocking_cr` inputs.
...
**Canonical routing:** This rule and its skip condition are rows 4 and 5 of the [PR status determination table](pr-status-table.md).
...
**Action:** Flag for human review. Do not auto-transition — PR comments are noisy (bots, CI, clarifying questions) and may not represent actionable feedback requiring rework. Present the comment author, timestamp, and a snippet of the comment body so the human can decide. (This is the comment-only caveat to row 4 of the [PR status determination table](pr-status-table.md): comment-only feedback moves Triage/In Progress PRs but only flags Awaiting Review PRs.)
