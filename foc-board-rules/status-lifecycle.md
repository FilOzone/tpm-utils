...
**Discovering unlinked PRs (In Progress issues only):** The "Linked pull requests" field only reflects formal GitHub closing references (`Closes #N`, `Fixes #N`). Contributors often use informal references instead ("Addresses #N", "Related to #N", or just mentioning `#N` in the PR body), which create cross-reference events but **not** formal links — so the board field stays empty. To catch these, apply a targeted check for **In Progress issues with no linked PRs**:
1. Identify candidates: `is:issue status:"⌨️ In Progress" no:linked-pull-requests` — In Progress issues with no formal linked PRs. Exclude zOrganizing Items.
2. Batch-fetch `timelineItems(itemTypes: [CROSS_REFERENCED_EVENT])` via GraphQL (general behavior rule 12) for the candidates only. This is an expensive query — do **not** run it for all issues, only In Progress ones with no formal linked PRs.
3. For each cross-referencing PR found: flag for human with the PR reference, its state (open/merged/closed), and whether it's on the board. The human should (a) add the PR to the board if missing, and (b) update the PR body to use `Closes #N` so future sweeps detect the link automatically.
