...
**When:** Any **issue or PR** on the board in "🎉 Done" has no Cycle set and was updated within the last 3 days (`updated:>@today-3d`).
**Action:** Set the Cycle to the current active cycle (the iteration whose date range contains today).
**Relationship to [R-FC-012](#r-fc-012-recently-active-items-without-a-cycle-get-the-current-cycle):** Same "no cycle -> assign current cycle" logic and the same 3-day window, but R-FC-012 explicitly excludes Done items — this rule is that gap's Done-side counterpart, catching items that moved straight to Done without ever getting a Cycle (e.g. via R-PR-008/R-PR-009's merged/closed-to-Done moves).
