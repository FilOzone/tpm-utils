...
**How the removal check works, and its limit:** GitHub has no change-history API for Projects v2 custom fields other than Status (confirmed by GraphQL schema introspection — only `ProjectV2ItemStatusChangedEvent` exists as a timeline event; `IssueFieldChangedEvent` and friends belong to a separate, unrelated "Issue Fields" GitHub feature, and returned no results when checked against a real item with a multi-cycle history). So this rule can only detect a removal it itself witnessed: it keeps its own mutation history (a small TSV file, persisted across hourly CI runs via GitHub Actions cache — see the workflow file) and checks that instead of GitHub. A cycle a human cleared *before* this rule ever set it (or before this history existed) won't be caught — the item will just look like any other item missing a Cycle and will get (re-)assigned.
