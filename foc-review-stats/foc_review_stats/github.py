...
def list_org_repos(
    session: requests.Session, org: str, pushed_since: str
) -> list[str]:
...
def fetch_repo_prs(
    session: requests.Session, name_with_owner: str, since_iso: str
) -> list[dict[str, Any]]:
...
def list_team_members(session: requests.Session, qualified_slug: str) -> list[str]:
...
def fetch_display_names(
    session: requests.Session, logins: Iterable[str], batch_size: int = 50
) -> dict[str, str]:
    ...
    """
    out: dict[str, str] = {}
    logins = list({l for l in logins if l})
